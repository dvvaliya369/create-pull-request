  ...
  try {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        if (attempt < 3) {
          core.warning(
            `Failed to prune remote '${branchRemoteName}' (attempt ${attempt}/3): ${errorMessage}. Retrying...`
          )
          // Wait 1 second before retrying
          await new Promise(resolve => setTimeout(resolve, 1000))
        } else {
          core.warning(
            `Failed to prune remote '${branchRemoteName}' after ${attempt} attempts: ${errorMessage}. Continuing anyway as this is a non-critical operation.`
          )
        }
      }
    }
