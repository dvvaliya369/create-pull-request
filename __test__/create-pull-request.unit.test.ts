...
describe('create-pull-request PR title and body defaulting', () => {
  ...
  test('should use commit subject as title for single commit when title is default', () => {
    ...
    if (branchCommits.length === 1) {
      ...
      if (body === defaultBody && singleCommit.body && singleCommit.body.trim().length > 0) {
  ...
  test('should handle multi-line commit body', () => {
    const commit: Commit = {
      sha: 'abc123',
      tree: 'tree123',
      parents: ['parent123'],
    ...
    expect(title).toEqual('Fix bug in parser')
    expect(body).toEqual('This commit fixes a critical bug.\n\nThe issue was in the parsing logic.\n\nFixes #123')
  })
})
